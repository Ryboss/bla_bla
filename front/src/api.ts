...
export const api = {
  ...
  trips: {
    ...
    filter: (filters: TripFiltersPayload, token: string) =>
      request<Trip[]>("/trips/filter", {
        method: "POST",
        token,
        body: filters,
      }),
    ...
    update: (tripId: number, payload: TripPayload, token: string) =>
  ...
  bookings: {
    ...
    listForTrip: (tripId: number, token: string) =>
      request<Booking[]>(`/trips/${tripId}/bookings`, { token }),
    create: (tripId: number, payload: BookingPayload, token: string) =>
    ...
    delete: (bookingId: number, token: string) =>
      request<Booking | boolean>(`/trips/bookings/${bookingId}`, {
        method: "DELETE",
        token,
      }),
  },
