...
export function TripDetailsPage() {
  ...
  async function handleUpdateTrip(event: FormEvent<HTMLFormElement>) {
    ...
    if (validationError || !token || !tripId) {
      setPageNotice(
        validationError ? { kind: "error", text: validationError } : null,
      );
      return;
    }
    ...
    try {
      const updatedTrip = await api.trips.update(
        tripId,
        buildTripPayload(editForm),
        token,
      );
      setTrip(updatedTrip);
      setEditForm({
        departure_city: updatedTrip.departure_city,
        arrival_city: updatedTrip.arrival_city,
        departure_time: toDatetimeLocalValue(updatedTrip.departure_time),
        arrival_time: toDatetimeLocalValue(updatedTrip.arrival_time),
        price: String(updatedTrip.price),
        available_seats: String(updatedTrip.available_seats),
      });
      setTrips((prev) =>
        prev.map((item) => (item.id === updatedTrip.id ? updatedTrip : item)),
      );
  ...
  async function handleDeleteTrip() {
    ...
    try {
      ...
      navigate("/trips", {
        replace: true,
        state: {
          notice: { kind: "success", text: `Поездка #${tripId} удалена.` },
        },
      });
    } catch (error) {
      setPageNotice({ kind: "error", text: getErrorMessage(error) });
  ...
  async function handleCreateBooking(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      navigate(`/bookings/${booking.id}`, {
        replace: true,
        state: {
          notice: { kind: "success", text: `Бронь #${booking.id} создана.` },
        },
      });
    } catch (error) {
      setPageNotice({ kind: "error", text: getErrorMessage(error) });
  ...
  return (
    <>
      <Section
        title={`Поездка #${trip.id}`}
        subtitle="Отдельная страница просмотра, редактирования и бронирования."
      >
        {pageNotice ? (
          <div className={`notice notice--${pageNotice.kind}`}>
            {pageNotice.text}
          </div>
        ) : null}
        <article className="panel panel-highlight">
          <div className="trip-card__top">
            ...
            <a
              ...
              href={buildYandexRouteUrl(trip.departure_city, trip.arrival_city)}
              target="_blank"
              rel="noreferrer"
            >
              Открыть маршрут
      ...
      <Section
        ...
        subtitle="После сохранения вы остаётесь на этой странице с обновлёнными данными."
      >
      ...
      <Section
        title="Брони по поездке"
        subtitle="Создание новой брони ведёт на отдельную страницу этой брони."
      >
        <div className="detail-grid">
          ...
          <div className="panel">
            <div className="section-head section-head--compact">
              <div>
                ...
                <p className="section-subtitle">
                  Каждая бронь открывается на отдельной странице.
                </p>
              </div>
            ...
            {!bookings.length ? (
              <p className="muted">Для этой поездки броней пока нет.</p>
            ) : null}
